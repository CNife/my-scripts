...
7. **观点归属判断**: 只总结无标记的本人发言内容，`//` 和 `>` 开头的内容仅作为上下文理解

### 观点区分规则

**重要**：只有不以 `//` 或 `>` 开头的内容才是发言人自己的观点。
...
**注释**：发言人内容中的 `回复@XXX：` 前缀是雪球自动生成的，整行（包括前缀）都是发言人自己写的文字，应总结为发言人观点。
...
**错误总结**：发言人讨论了支付宝音响、中概股财报（× 这些是他人的观点）
